...
class Mouse:
    ...
    def _update_movement(self, left_enc: int, right_enc: int):
        ...
        """
        diff = (left_enc * constants.ENCODER_TO_MM + right_enc * constants.ENCODER_TO_MM) / 2
        self.speed = diff / 0.1
        self.pos += diff
    ...
    def update_sensor_data(self, init_update: bool = False):
        ...
        if self._sim:
            ...
            imu = sensor_data['imu']
            cur_angle = (imu["yaw"] - self.angle_error) % 360

            if init_update:
                self.angle_error = cur_angle
                self.angle = 0
                cur_angle = 0
...
def check_turns():
    ...
    print(mouse)
    mouse.right()
    print(f'{mouse.reference_angle}, real={mouse.angle} diff={mouse.reference_angle - mouse.angle}')
    mouse.right()
    mouse.update_sensor_data()
    print(f'{mouse.reference_angle}, real={mouse.angle} diff={mouse.reference_angle - mouse.angle}')
